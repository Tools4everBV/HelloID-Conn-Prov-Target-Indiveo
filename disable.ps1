##################################################
# HelloID-Conn-Prov-Target-Indiveo-Disable
# PowerShell V2
##################################################
...
try {
    ...
    switch ($action) {
        'DisableAccount' {
            if (-not($actionContext.DryRun -eq $true)) {
                ...
                Write-Information "[DryRun] Disable Indiveo account with accountReference: [$($actionContext.References.Account)], will be executed during enforcement"
            }
    ...
    if ($($ex.Exception.GetType().FullName -eq 'Microsoft.PowerShell.Commands.HttpResponseException') -or
        ...
        $auditLogMessage = "Could not disable Indiveo account. Error: $($errorObj.FriendlyMessage). Action initiated by: [$($actionContext.Origin)]"
        Write-Warning "Error at Line '$($errorObj.ScriptLineNumber)': $($errorObj.Line). Error: $($errorObj.ErrorDetails)"
    }
    else {
        $auditLogMessage = "Could not disable Indiveo account. Error: $($_.Exception.Message). Action initiated by: [$($actionContext.Origin)]"
